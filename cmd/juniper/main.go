// Package main provides a standalone 'juniper' command for Bible/SWORD module tools.
// This is a thin wrapper for backwards compatibility with projects that expect
// a standalone 'juniper' binary instead of 'capsule-juniper' or 'capsule juniper'.
//
// Usage:
//
//	juniper list [--path ~/.sword]
//	juniper ingest --all [--path ~/.sword] [--output ./capsules]
//	juniper cas-to-sword capsule.tar.gz [--output ~/.sword] [--name MODULE]
//	juniper repoman <command> [options]
//
// This is equivalent to running 'capsule-juniper' with the same arguments.
package main
...
func printUsage() {
	fmt.Println(`juniper - Bible/SWORD module tools

Usage:
  juniper <command> [options]
...
Examples:
  juniper install KJV ASV         Install specific modules with IR
  juniper install --all           Install all Bible modules with IR
  juniper hugo KJV ASV DRC        Export specific modules
  juniper hugo --all              Export all Bible modules

Run 'juniper repoman help' for repoman subcommands.

Note: This is equivalent to 'capsule-juniper' or 'capsule juniper'.`)
}
