...
func parse(path string) (*ir.Corpus, error) {
	...
	sourceHash, err := computeSourceHash(path)
	if err != nil {
		return nil, err
	}
	...
	corpus.LossClass = "L1"
	corpus.SourceHash = sourceHash

	extractByExtension(db, corpus, ext)
	applyDetailsMetadata(db, corpus)
...
func extractByExtension(db *sql.DB, corpus *ir.Corpus, ext string) {
	switch ext {
	case ".bblx":
...
func applyDetailsMetadata(db *sql.DB, corpus *ir.Corpus) {
...
func extractBibleIR(db *sql.DB, corpus *ir.Corpus) {
	...
	for rows.Next() {
		...
		sequence++
		doc.ContentBlocks = append(doc.ContentBlocks, createBibleContentBlock(bookNum, chapter, verse, stripRTF(scripture), sequence))
	}

	appendDocsInOrder(corpus, bookDocs)
}

// getOrCreateBookDoc gets or creates a document for a book number
...
// createBibleContentBlock creates a content block for a verse
func createBibleContentBlock(bookNum, chapter, verse int, text string, sequence int) *ir.ContentBlock {
	osisID := bookNumToOSIS[bookNum]
	if osisID == "" {
		osisID = fmt.Sprintf("Book%d", bookNum)
	}
	...
	return &ir.ContentBlock{
		ID: fmt.Sprintf("cb-%d", sequence), Sequence: sequence, Text: text, Hash: hex.EncodeToString(hash[:]),
		Anchors: []*ir.Anchor{{
			ID: fmt.Sprintf("a-%d-0", sequence), Position: 0,
			Spans: []*ir.Span{{
				ID: fmt.Sprintf("s-%s", refID), Type: "VERSE", StartAnchorID: fmt.Sprintf("a-%d-0", sequence),
...
// appendDocsInOrder adds documents to corpus in book order
func appendDocsInOrder(corpus *ir.Corpus, bookDocs map[int]*ir.Document) {
	for i := 1; i <= 66; i++ {
...
var eswordExtensions = map[string]string{
	"COMMENTARY": ".cmtx",
	"DICTIONARY": ".dctx",
}

func getEswordExtension(moduleType string) string {
	if ext, ok := eswordExtensions[moduleType]; ok {
		return ext
	}
	return ".bblx"
}
...
func emit(corpus *ir.Corpus, outputDir string) (string, error) {
	ext := getEswordExtension(string(corpus.ModuleType))
	...
	err = sqlite.WithTransaction(context.Background(), db, func(tx *sql.Tx) error {
		if err := emitCorpusContent(tx, corpus); err != nil {
			return err
		}

		if err := createDetailsTable(tx, corpus); err != nil {
			return err
		}
		return nil
	})
...
func emitCorpusContent(tx *sql.Tx, corpus *ir.Corpus) error {
	emitters := map[string]func(*sql.Tx, *ir.Corpus) error{
	...
	if err := emitFn(tx, corpus); err != nil {
		return fmt.Errorf("failed to emit content: %w", err)
	}
	return nil
}

func createDetailsTable(tx *sql.Tx, corpus *ir.Corpus) error {
	if _, err := tx.Exec("CREATE TABLE IF NOT EXISTS Details (Title TEXT, Abbreviation TEXT, Information TEXT, Version TEXT, Font TEXT, RightToLeft INTEGER)"); err != nil {
	...
	if _, err := tx.Exec("INSERT INTO Details (Title, Abbreviation, Information, Version, RightToLeft) VALUES (?, ?, ?, ?, ?)",
...
func emitBibleNative(tx *sql.Tx, corpus *ir.Corpus) error {
	if _, err := tx.Exec("CREATE TABLE IF NOT EXISTS Bible (Book INTEGER, Chapter INTEGER, Verse INTEGER, Scripture TEXT)"); err != nil {
		return fmt.Errorf("create Bible table: %w", err)
	}

	for _, doc := range corpus.Documents {
		if err := emitBibleDocument(tx, doc); err != nil {
			return err
		}
	}
	return nil
...
func getBookNum(doc *ir.Document) int {
	...
	if num, ok := osisToBookNum[doc.ID]; ok {
		return num
	}
	return 0
}

func emitBibleDocument(tx *sql.Tx, doc *ir.Document) error {
	bookNum := getBookNum(doc)
	for _, cb := range doc.ContentBlocks {
		if err := emitBibleContentBlock(tx, doc.ID, bookNum, cb); err != nil {
			return err
		}
	}
	return nil
...
func emitBibleContentBlock(tx *sql.Tx, docID string, bookNum int, cb *ir.ContentBlock) error {
...
func emitCommentaryNative(tx *sql.Tx, corpus *ir.Corpus) error {
	if _, err := tx.Exec("CREATE TABLE IF NOT EXISTS Commentary (Book INTEGER, ChapterBegin INTEGER, ChapterEnd INTEGER, VerseBegin INTEGER, VerseEnd INTEGER, Comments TEXT)"); err != nil {
		return fmt.Errorf("create Commentary table: %w", err)
	}
	for _, doc := range corpus.Documents {
		if err := emitDocumentCommentaries(tx, doc); err != nil {
			return err
		}
	}
	return nil
...
func emitDocumentCommentaries(tx *sql.Tx, doc *ir.Document) error {
	for _, cb := range doc.ContentBlocks {
		if err := emitContentBlockCommentaries(tx, cb); err != nil {
			return err
		}
	}
	return nil
...
func emitContentBlockCommentaries(tx *sql.Tx, cb *ir.ContentBlock) error {
	for _, anchor := range cb.Anchors {
		for _, span := range anchor.Spans {
			if span.Ref == nil {
				continue
...
func emitDictionaryNative(tx *sql.Tx, corpus *ir.Corpus) error {
	if _, err := tx.Exec("CREATE TABLE IF NOT EXISTS Dictionary (Topic TEXT, Definition TEXT)"); err != nil {
