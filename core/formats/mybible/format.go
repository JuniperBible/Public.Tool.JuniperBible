// Package mybible implements the MyBible.zone Bible format.
...
func extractBibleIR(db *sql.DB, corpus *ir.Corpus) {
	...
	for rows.Next() {
		...
		sequence++
		doc.ContentBlocks = append(doc.ContentBlocks, createMyBibleContentBlock(bookNum, chapter, verse, stripHTML(text), sequence))
	}
...
// createMyBibleContentBlock creates a content block for a verse
func createMyBibleContentBlock(bookNum, chapter, verse int, text string, sequence int) *ir.ContentBlock {
	osisID := bookNumToOSIS[bookNum]
	if osisID == "" {
		osisID = fmt.Sprintf("Book%d", bookNum)
	}
...
func emitBibleNative(tx *sql.Tx, corpus *ir.Corpus) error {
	for _, doc := range corpus.Documents {
		...
		if err := emitDocumentVerses(tx, doc, bookNum); err != nil {
			return err
		}
	}
	return nil
...
func emitDocumentVerses(tx *sql.Tx, doc *ir.Document, bookNum int) error {
	for _, cb := range doc.ContentBlocks {
		if err := emitContentBlockVerses(tx, doc.ID, cb, bookNum); err != nil {
			return err
		}
	}
	return nil
...
func emitContentBlockVerses(tx *sql.Tx, docID string, cb *ir.ContentBlock, bookNum int) error {
