// Package accordance provides the embedded handler for Accordance format plugin.
package accordance
...
// Handler implements the EmbeddedFormatHandler interface for Accordance.
type Handler struct{}
...
func Manifest() *plugins.PluginManifest {
	return &plugins.PluginManifest{
		...
		Version:    "1.0.0",
		Kind:       "format",
		Entrypoint: "format-accordance",
		Capabilities: plugins.Capabilities{
			Inputs:  []string{"file"},
			Outputs: []string{"artifact.kind:accordance"},
		},
	}
}
...
func (h *Handler) Detect(path string) (*plugins.DetectResult, error) {
	...
	if ext != ".accordance" {
		return &plugins.DetectResult{Detected: false, Reason: "not a .accordance file"}, nil
	}

	return &plugins.DetectResult{
		Detected: true,
		Format:   "accordance",
		Reason:   "Accordance file detected",
	}, nil
}
...
func (h *Handler) ExtractIR(path, outputDir string) (*plugins.ExtractIRResult, error) {
	return nil, fmt.Errorf("accordance format does not support IR extraction")
}
...
func (h *Handler) EmitNative(irPath, outputDir string) (*plugins.EmitNativeResult, error) {
	return nil, fmt.Errorf("accordance format does not support native emission")
}
