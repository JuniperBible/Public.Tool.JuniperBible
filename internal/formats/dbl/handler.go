// Package dbl provides the embedded handler for DBL format plugin.
package dbl
...
// Handler implements the EmbeddedFormatHandler interface for DBL.
type Handler struct{}
...
func (h *Handler) Detect(path string) (*plugins.DetectResult, error) {
	...
	if ext != ".dbl" {
		return &plugins.DetectResult{Detected: false, Reason: "not a .dbl file"}, nil
	}

	return &plugins.DetectResult{
		Detected: true,
		Format:   "dbl",
		Reason:   "DBL file detected",
	}, nil
}
...
func (h *Handler) ExtractIR(path, outputDir string) (*plugins.ExtractIRResult, error) {
	return nil, fmt.Errorf("dbl format does not support IR extraction")
}
...
func (h *Handler) EmitNative(irPath, outputDir string) (*plugins.EmitNativeResult, error) {
	return nil, fmt.Errorf("dbl format does not support native emission")
}
