// Package epub provides the embedded handler for EPUB format plugin.
package epub
...
// Handler implements the EmbeddedFormatHandler interface for EPUB.
type Handler struct{}
...
func Manifest() *plugins.PluginManifest {
	return &plugins.PluginManifest{
		...
		Version:    "1.0.0",
		Kind:       "format",
		Entrypoint: "format-epub",
		Capabilities: plugins.Capabilities{
			Inputs:  []string{"file"},
			Outputs: []string{"artifact.kind:epub"},
		},
	}
}
...
func (h *Handler) Detect(path string) (*plugins.DetectResult, error) {
	...
	if ext != ".epub" {
		return &plugins.DetectResult{Detected: false, Reason: "not a .epub file"}, nil
	}

	return &plugins.DetectResult{
		Detected: true,
		Format:   "epub",
		Reason:   "EPUB file detected",
	}, nil
}
...
func (h *Handler) ExtractIR(path, outputDir string) (*plugins.ExtractIRResult, error) {
	return nil, fmt.Errorf("epub format does not support IR extraction")
}
...
func (h *Handler) EmitNative(irPath, outputDir string) (*plugins.EmitNativeResult, error) {
	return nil, fmt.Errorf("epub format does not support native emission")
}
