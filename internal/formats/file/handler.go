// Package file provides the embedded handler for generic file format plugin.
package file
...
// Handler implements the EmbeddedFormatHandler interface for generic files.
type Handler struct{}
...
func Manifest() *plugins.PluginManifest {
	return &plugins.PluginManifest{
		...
		Capabilities: plugins.Capabilities{
			Inputs:  []string{"file"},
			Outputs: []string{"artifact.kind:file"},
		},
	}
}
...
func (h *Handler) Ingest(path, outputDir string) (*plugins.IngestResult, error) {
	...
	return &plugins.IngestResult{
		...
		Metadata:   map[string]string{"format": "file"},
	}, nil
}
...
func (h *Handler) ExtractIR(path, outputDir string) (*plugins.ExtractIRResult, error) {
	return nil, fmt.Errorf("generic file format does not support IR extraction")
}
...
func (h *Handler) EmitNative(irPath, outputDir string) (*plugins.EmitNativeResult, error) {
	return nil, fmt.Errorf("generic file format does not support native emission")
}
