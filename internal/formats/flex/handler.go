// Package flex provides the embedded handler for FLEx format plugin.
package flex
...
// Handler implements the EmbeddedFormatHandler interface for FLEx.
type Handler struct{}
...
func Manifest() *plugins.PluginManifest {
	return &plugins.PluginManifest{
		...
		Version:    "1.0.0",
		Kind:       "format",
		Entrypoint: "format-flex",
		Capabilities: plugins.Capabilities{
			Inputs:  []string{"file"},
			Outputs: []string{"artifact.kind:flex"},
		},
	}
}
...
func (h *Handler) Detect(path string) (*plugins.DetectResult, error) {
	...
	if ext != ".flextext" {
		return &plugins.DetectResult{Detected: false, Reason: "not a .flextext file"}, nil
	}

	return &plugins.DetectResult{
		Detected: true,
		Format:   "flex",
		Reason:   "FLEx file detected",
	}, nil
}
...
func (h *Handler) ExtractIR(path, outputDir string) (*plugins.ExtractIRResult, error) {
	return nil, fmt.Errorf("flex format does not support IR extraction")
}
...
func (h *Handler) EmitNative(irPath, outputDir string) (*plugins.EmitNativeResult, error) {
	return nil, fmt.Errorf("flex format does not support native emission")
}
