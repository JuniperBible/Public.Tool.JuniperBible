// Package gobible provides the embedded handler for GoBible format plugin.
package gobible
...
// Handler implements the EmbeddedFormatHandler interface for GoBible.
type Handler struct{}
...
func Manifest() *plugins.PluginManifest {
	return &plugins.PluginManifest{
		...
		Version:    "1.0.0",
		Kind:       "format",
		Entrypoint: "format-gobible",
		Capabilities: plugins.Capabilities{
			Inputs:  []string{"file"},
			Outputs: []string{"artifact.kind:gobible"},
		},
	}
}
...
func (h *Handler) Detect(path string) (*plugins.DetectResult, error) {
	...
	if ext != ".jar" {
		return &plugins.DetectResult{Detected: false, Reason: "not a .jar file"}, nil
	}

	return &plugins.DetectResult{
		Detected: true,
		Format:   "gobible",
		Reason:   "GoBible file detected",
	}, nil
}
...
func (h *Handler) ExtractIR(path, outputDir string) (*plugins.ExtractIRResult, error) {
	return nil, fmt.Errorf("gobible format does not support IR extraction")
}
...
func (h *Handler) EmitNative(irPath, outputDir string) (*plugins.EmitNativeResult, error) {
	return nil, fmt.Errorf("gobible format does not support native emission")
}
