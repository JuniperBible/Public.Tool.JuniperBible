// Package logos provides the embedded handler for Logos format plugin.
package logos
...
// Handler implements the EmbeddedFormatHandler interface for Logos.
type Handler struct{}
...
func Manifest() *plugins.PluginManifest {
	return &plugins.PluginManifest{
		...
		Version:    "1.0.0",
		Kind:       "format",
		Entrypoint: "format-logos",
		Capabilities: plugins.Capabilities{
			Inputs:  []string{"file"},
			Outputs: []string{"artifact.kind:logos"},
		},
	}
}
...
func (h *Handler) Detect(path string) (*plugins.DetectResult, error) {
	...
	if ext != ".logos" {
		return &plugins.DetectResult{Detected: false, Reason: "not a .logos file"}, nil
	}

	return &plugins.DetectResult{
		Detected: true,
		Format:   "logos",
		Reason:   "Logos file detected",
	}, nil
}
...
func (h *Handler) ExtractIR(path, outputDir string) (*plugins.ExtractIRResult, error) {
	return nil, fmt.Errorf("logos format does not support IR extraction")
}
...
func (h *Handler) EmitNative(irPath, outputDir string) (*plugins.EmitNativeResult, error) {
	return nil, fmt.Errorf("logos format does not support native emission")
}
