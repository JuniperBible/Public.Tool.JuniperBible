// Package markdown provides the embedded handler for Markdown format plugin.
package markdown
...
// Handler implements the EmbeddedFormatHandler interface for Markdown.
type Handler struct{}
...
func Manifest() *plugins.PluginManifest {
	return &plugins.PluginManifest{
		...
		Version:    "1.0.0",
		Kind:       "format",
		Entrypoint: "format-markdown",
		Capabilities: plugins.Capabilities{
			Inputs:  []string{"file"},
			Outputs: []string{"artifact.kind:markdown"},
		},
	}
}
...
func (h *Handler) Detect(path string) (*plugins.DetectResult, error) {
	...
	if ext != ".md" {
		return &plugins.DetectResult{Detected: false, Reason: "not a .md file"}, nil
	}

	return &plugins.DetectResult{
		Detected: true,
		Format:   "markdown",
		Reason:   "Markdown file detected",
	}, nil
}
...
func (h *Handler) ExtractIR(path, outputDir string) (*plugins.ExtractIRResult, error) {
	return nil, fmt.Errorf("markdown format does not support IR extraction")
}
...
func (h *Handler) EmitNative(irPath, outputDir string) (*plugins.EmitNativeResult, error) {
	return nil, fmt.Errorf("markdown format does not support native emission")
}
