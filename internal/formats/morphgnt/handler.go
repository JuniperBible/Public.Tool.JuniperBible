// Package morphgnt provides the embedded handler for MorphGNT format plugin.
package morphgnt
...
// Handler implements the EmbeddedFormatHandler interface for MorphGNT.
type Handler struct{}
...
func Manifest() *plugins.PluginManifest {
	return &plugins.PluginManifest{
		...
		Version:    "1.0.0",
		Kind:       "format",
		Entrypoint: "format-morphgnt",
		Capabilities: plugins.Capabilities{
			Inputs:  []string{"file"},
			Outputs: []string{"artifact.kind:morphgnt"},
		},
	}
}
...
func (h *Handler) Detect(path string) (*plugins.DetectResult, error) {
	...
	if ext != ".morphgnt" {
		return &plugins.DetectResult{Detected: false, Reason: "not a .morphgnt file"}, nil
	}

	return &plugins.DetectResult{
		Detected: true,
		Format:   "morphgnt",
		Reason:   "MorphGNT file detected",
	}, nil
}
...
func (h *Handler) ExtractIR(path, outputDir string) (*plugins.ExtractIRResult, error) {
	return nil, fmt.Errorf("morphgnt format does not support IR extraction")
}
...
func (h *Handler) EmitNative(irPath, outputDir string) (*plugins.EmitNativeResult, error) {
	return nil, fmt.Errorf("morphgnt format does not support native emission")
}
