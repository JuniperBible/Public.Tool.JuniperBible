...
func FuzzStripHTML(f *testing.F) {
	...
	f.Add("Text<sup>1</sup>with<sub>2</sub>superscripts")
...
func FuzzVerse(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, book, chapter, verse int, text string) {
...
// FuzzDetectModuleType tests the module type detection with fuzzing
func FuzzDetectModuleType(f *testing.F) {
	// Seed with various filename patterns
	f.Add("bible.mybible")
	f.Add("commentary.commentaries.mybible")
	f.Add("dictionary.dictionary.mybible")
	f.Add("BIBLE.MYBIBLE")
	f.Add("test.COMMENTARIES.MYBIBLE")
	f.Add("example.DICTIONARY.MYBIBLE")
	f.Add("invalid.txt")
	f.Add("")
	f.Add(".mybible")
	f.Add("mybible")
	f.Add("file.mybible.backup")
	f.Add("path/to/file.mybible")
	f.Add("C:\\Windows\\file.mybible")

	f.Fuzz(func(t *testing.T, filename string) {
		// DetectModuleType should not panic on any input
		moduleType := DetectModuleType(filename)
		...
		if !validTypes[moduleType] {
			t.Errorf("Unexpected module type: %s", moduleType)
		}

		// Verify specific patterns
		if len(filename) > 0 {
			lower := filename
			// Simple lowercase check (mimicking the function behavior)
			hasMyBible := false
			for i := 0; i < len(lower)-7; i++ {
				if lower[i:i+8] == ".mybible" || lower[i:i+8] == ".MYBIBLE" {
					hasMyBible = true
					break
				}
			}

			// If filename contains .mybible extension, type should not be empty
			// (unless it's just ".mybible" or has other issues)
			if hasMyBible && len(filename) > 8 {
				_ = moduleType // Type may still be "" for malformed filenames
			}
		}
	})
}
...
func FuzzParseMetadata(f *testing.F) {
	...
	f.Add("publisher", "Public Domain")
	f.Add("copyright", "©2024")
