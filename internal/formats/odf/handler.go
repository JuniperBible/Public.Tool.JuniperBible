// Package odf provides the embedded handler for ODF format plugin.
package odf
...
// Handler implements the EmbeddedFormatHandler interface for ODF.
type Handler struct{}
...
func (h *Handler) Detect(path string) (*plugins.DetectResult, error) {
	...
	if ext != ".odt" {
		return &plugins.DetectResult{Detected: false, Reason: "not a .odt file"}, nil
	}

	return &plugins.DetectResult{
		Detected: true,
		Format:   "odf",
		Reason:   "ODF file detected",
	}, nil
}
...
func (h *Handler) ExtractIR(path, outputDir string) (*plugins.ExtractIRResult, error) {
	return nil, fmt.Errorf("odf format does not support IR extraction")
}
...
func (h *Handler) EmitNative(irPath, outputDir string) (*plugins.EmitNativeResult, error) {
	return nil, fmt.Errorf("odf format does not support native emission")
}
