// Package onlinebible provides the embedded handler for Online Bible format plugin.
package onlinebible
...
// Handler implements the EmbeddedFormatHandler interface for Online Bible.
type Handler struct{}
...
func Manifest() *plugins.PluginManifest {
	return &plugins.PluginManifest{
		...
		Version:    "1.0.0",
		Kind:       "format",
		Entrypoint: "format-onlinebible",
		Capabilities: plugins.Capabilities{
			Inputs:  []string{"file"},
			Outputs: []string{"artifact.kind:onlinebible"},
		},
	}
}
...
func (h *Handler) Detect(path string) (*plugins.DetectResult, error) {
	...
	if ext != ".olb" {
		return &plugins.DetectResult{Detected: false, Reason: "not a .olb file"}, nil
	}

	return &plugins.DetectResult{
		Detected: true,
		Format:   "onlinebible",
		Reason:   "Online Bible file detected",
	}, nil
}
...
func (h *Handler) ExtractIR(path, outputDir string) (*plugins.ExtractIRResult, error) {
	return nil, fmt.Errorf("onlinebible format does not support IR extraction")
}
...
func (h *Handler) EmitNative(irPath, outputDir string) (*plugins.EmitNativeResult, error) {
	return nil, fmt.Errorf("onlinebible format does not support native emission")
}
