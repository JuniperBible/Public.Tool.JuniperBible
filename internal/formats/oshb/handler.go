// Package oshb provides the embedded handler for OSHB format plugin.
package oshb
...
// Handler implements the EmbeddedFormatHandler interface for OSHB.
type Handler struct{}
...
func Manifest() *plugins.PluginManifest {
	return &plugins.PluginManifest{
		...
		Version:    "1.0.0",
		Kind:       "format",
		Entrypoint: "format-oshb",
		Capabilities: plugins.Capabilities{
			Inputs:  []string{"file"},
			Outputs: []string{"artifact.kind:oshb"},
		},
	}
}
...
func (h *Handler) Detect(path string) (*plugins.DetectResult, error) {
	...
	if ext != ".oshb" {
		return &plugins.DetectResult{Detected: false, Reason: "not a .oshb file"}, nil
	}

	return &plugins.DetectResult{
		Detected: true,
		Format:   "oshb",
		Reason:   "OSHB file detected",
	}, nil
}
...
func (h *Handler) ExtractIR(path, outputDir string) (*plugins.ExtractIRResult, error) {
	return nil, fmt.Errorf("oshb format does not support IR extraction")
}
...
func (h *Handler) EmitNative(irPath, outputDir string) (*plugins.EmitNativeResult, error) {
	return nil, fmt.Errorf("oshb format does not support native emission")
}
