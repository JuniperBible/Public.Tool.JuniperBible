// Package sblgnt provides the embedded handler for SBLGNT format plugin.
package sblgnt
...
// Handler implements the EmbeddedFormatHandler interface for SBLGNT.
type Handler struct{}
...
func Manifest() *plugins.PluginManifest {
	return &plugins.PluginManifest{
		...
		Version:    "1.0.0",
		Kind:       "format",
		Entrypoint: "format-sblgnt",
		Capabilities: plugins.Capabilities{
			Inputs:  []string{"file"},
			Outputs: []string{"artifact.kind:sblgnt"},
		},
	}
}
...
func (h *Handler) Detect(path string) (*plugins.DetectResult, error) {
	...
	if ext != ".sblgnt" {
		return &plugins.DetectResult{Detected: false, Reason: "not a .sblgnt file"}, nil
	}

	return &plugins.DetectResult{
		Detected: true,
		Format:   "sblgnt",
		Reason:   "SBLGNT file detected",
	}, nil
}
...
func (h *Handler) ExtractIR(path, outputDir string) (*plugins.ExtractIRResult, error) {
	return nil, fmt.Errorf("sblgnt format does not support IR extraction")
}
...
func (h *Handler) EmitNative(irPath, outputDir string) (*plugins.EmitNativeResult, error) {
	return nil, fmt.Errorf("sblgnt format does not support native emission")
}
