// Package sfm provides the embedded handler for SFM format plugin.
package sfm
...
// Handler implements the EmbeddedFormatHandler interface for SFM.
type Handler struct{}
...
func Manifest() *plugins.PluginManifest {
	return &plugins.PluginManifest{
		...
		Version:    "1.0.0",
		Kind:       "format",
		Entrypoint: "format-sfm",
		Capabilities: plugins.Capabilities{
			Inputs:  []string{"file"},
			Outputs: []string{"artifact.kind:sfm"},
		},
	}
}
...
func (h *Handler) Detect(path string) (*plugins.DetectResult, error) {
	...
	if ext != ".sfm" {
		return &plugins.DetectResult{Detected: false, Reason: "not a .sfm file"}, nil
	}

	return &plugins.DetectResult{
		Detected: true,
		Format:   "sfm",
		Reason:   "SFM file detected",
	}, nil
}
...
func (h *Handler) ExtractIR(path, outputDir string) (*plugins.ExtractIRResult, error) {
	return nil, fmt.Errorf("sfm format does not support IR extraction")
}
...
func (h *Handler) EmitNative(irPath, outputDir string) (*plugins.EmitNativeResult, error) {
	return nil, fmt.Errorf("sfm format does not support native emission")
}
