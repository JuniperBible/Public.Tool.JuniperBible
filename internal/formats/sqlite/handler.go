// Package sqlite provides the embedded handler for SQLite format plugin.
package sqlite
...
// Handler implements the EmbeddedFormatHandler interface for SQLite.
type Handler struct{}
...
func Manifest() *plugins.PluginManifest {
	return &plugins.PluginManifest{
		...
		Version:    "1.0.0",
		Kind:       "format",
		Entrypoint: "format-sqlite",
		Capabilities: plugins.Capabilities{
			Inputs:  []string{"file"},
			Outputs: []string{"artifact.kind:sqlite"},
		},
	}
}
...
func (h *Handler) Detect(path string) (*plugins.DetectResult, error) {
	...
	if ext != ".sqlite" {
		return &plugins.DetectResult{Detected: false, Reason: "not a .sqlite file"}, nil
	}

	return &plugins.DetectResult{
		Detected: true,
		Format:   "sqlite",
		Reason:   "SQLite file detected",
	}, nil
}
...
func (h *Handler) Ingest(path, outputDir string) (*plugins.IngestResult, error) {
	...
	return &plugins.IngestResult{
		...
		Metadata:   map[string]string{"format": "sqlite"},
	}, nil
}
...
func (h *Handler) ExtractIR(path, outputDir string) (*plugins.ExtractIRResult, error) {
	return nil, fmt.Errorf("sqlite format does not support IR extraction")
}
...
func (h *Handler) EmitNative(irPath, outputDir string) (*plugins.EmitNativeResult, error) {
	return nil, fmt.Errorf("sqlite format does not support native emission")
}
