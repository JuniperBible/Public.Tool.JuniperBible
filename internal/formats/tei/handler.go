// Package tei provides the embedded handler for TEI format plugin.
package tei
...
// Handler implements the EmbeddedFormatHandler interface for TEI.
type Handler struct{}
...
func Manifest() *plugins.PluginManifest {
	return &plugins.PluginManifest{
		...
		Version:    "1.0.0",
		Kind:       "format",
		Entrypoint: "format-tei",
		Capabilities: plugins.Capabilities{
			Inputs:  []string{"file"},
			Outputs: []string{"artifact.kind:tei"},
		},
	}
}
...
func (h *Handler) Detect(path string) (*plugins.DetectResult, error) {
	...
	if ext != ".tei" {
		return &plugins.DetectResult{Detected: false, Reason: "not a .tei file"}, nil
	}

	return &plugins.DetectResult{
		Detected: true,
		Format:   "tei",
		Reason:   "TEI file detected",
	}, nil
}
...
func (h *Handler) ExtractIR(path, outputDir string) (*plugins.ExtractIRResult, error) {
	return nil, fmt.Errorf("tei format does not support IR extraction")
}
...
func (h *Handler) EmitNative(irPath, outputDir string) (*plugins.EmitNativeResult, error) {
	return nil, fmt.Errorf("tei format does not support native emission")
}
