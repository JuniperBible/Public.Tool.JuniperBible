...
func TestManifest(t *testing.T) {
	m := Manifest()
	if m.PluginID != "format.theword" {
		t.Errorf("Expected PluginID 'format.theword', got %s", m.PluginID)
	}
	if m.Kind != "format" {
...
func TestDetect_TheWordFile(t *testing.T) {
	tmpDir := t.TempDir()
	h := &Handler{}
	...
	if result.Format != "theword" {
		t.Errorf("Expected format 'theword', got %s", result.Format)
	}
}

func TestDetect_NonTheWordFile(t *testing.T) {
	tmpDir := t.TempDir()
	h := &Handler{}
	...
	if result.Detected {
		t.Error("Expected non-TheWord file to not be detected")
	}
}
...
func TestDetect_NonExistentFile(t *testing.T) {
	...
	result, err := h.Detect("/nonexistent/file.ont")
	if err != nil {
		t.Fatal(err)
	}
...
func TestIngest_TheWordFile(t *testing.T) {
	tmpDir := t.TempDir()
	h := &Handler{}
	...
	if result.Metadata["format"] != "theword" {
		t.Errorf("Expected format 'theword', got %s", result.Metadata["format"])
	}
...
func TestIngest_NonExistentFile(t *testing.T) {
	...
	_, err := h.Ingest("/nonexistent/file.ont", outputDir)
	if err == nil {
...
func TestEnumerate_TheWordFile(t *testing.T) {
	tmpDir := t.TempDir()
	h := &Handler{}
	...
	if entry.Path != "test.ont" {
		t.Errorf("Expected path 'test.ont', got %s", entry.Path)
	}
	if entry.IsDir {
...
func TestEnumerate_NonExistentFile(t *testing.T) {
	...
	_, err := h.Enumerate("/nonexistent/file.ont")
	if err == nil {
		t.Error("Expected error for non-existent file")
	}
...
func TestExtractIR(t *testing.T) {
	...
	_, err := h.ExtractIR("test.ont", "/output")
	if err == nil {
