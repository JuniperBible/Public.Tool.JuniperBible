...
func TestManifest(t *testing.T) {
	m := Manifest()
	if m.PluginID != "format.xml" {
		t.Errorf("Expected PluginID 'format.xml', got %s", m.PluginID)
	}
	if m.Kind != "format" {
...
func TestDetect_XMLFile(t *testing.T) {
	tmpDir := t.TempDir()
	h := &Handler{}
...
func TestIngest_XMLFile(t *testing.T) {
	tmpDir := t.TempDir()
	h := &Handler{}
	...
	if result.Metadata["format"] != "xml" {
		t.Errorf("Expected format 'xml', got %s", result.Metadata["format"])
	}
...
func TestEnumerate_XMLFile(t *testing.T) {
	tmpDir := t.TempDir()
	h := &Handler{}
