...
func TestManifest(t *testing.T) {
	m := Manifest()
	if m.PluginID != "format.zefania" {
		t.Errorf("Expected PluginID 'format.zefania', got %s", m.PluginID)
	}
	if m.Kind != "format" {
...
func TestDetect_ZefaniaFile(t *testing.T) {
	tmpDir := t.TempDir()
	h := &Handler{}
	...
	if result.Format != "zefania" {
		t.Errorf("Expected format 'zefania', got %s", result.Format)
	}
}

func TestDetect_NonXMLFile(t *testing.T) {
	tmpDir := t.TempDir()
	h := &Handler{}
	...
	if result.Detected {
		t.Error("Expected non-XML file to not be detected")
	}
}
...
func TestDetect_NonExistentFile(t *testing.T) {
	...
	result, err := h.Detect("/nonexistent/file.xml")
	if err != nil {
		t.Fatal(err)
	}
...
func TestIngest_ZefaniaFile(t *testing.T) {
	tmpDir := t.TempDir()
	h := &Handler{}

	file := filepath.Join(tmpDir, "test.xml")
	content := []byte("<?xml version=\"1.0\"?><XMLBIBLE>test</XMLBIBLE>")
	...
	if result.Metadata["format"] != "zefania" {
		t.Errorf("Expected format 'zefania', got %s", result.Metadata["format"])
	}
...
func TestIngest_NonExistentFile(t *testing.T) {
	...
	_, err := h.Ingest("/nonexistent/file.xml", outputDir)
	if err == nil {
...
func TestEnumerate_ZefaniaFile(t *testing.T) {
	tmpDir := t.TempDir()
	h := &Handler{}
	...
	if entry.Path != "test.xml" {
		t.Errorf("Expected path 'test.xml', got %s", entry.Path)
	}
	if entry.IsDir {
...
func TestEnumerate_NonExistentFile(t *testing.T) {
	...
	_, err := h.Enumerate("/nonexistent/file.xml")
	if err == nil {
		t.Error("Expected error for non-existent file")
	}
...
func TestExtractIR(t *testing.T) {
	...
	_, err := h.ExtractIR("test.xml", "/output")
	if err == nil {
