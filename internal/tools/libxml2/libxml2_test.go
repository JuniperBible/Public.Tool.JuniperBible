...
func TestManifest(t *testing.T) {
	...
	// Test PluginID
	if manifest.PluginID != "tool.libxml2" {
		t.Errorf("expected PluginID 'tool.libxml2', got '%s'", manifest.PluginID)
	}
	...
	// Test Entrypoint
	if manifest.Entrypoint != "tool-libxml2" {
		t.Errorf("expected Entrypoint 'tool-libxml2', got '%s'", manifest.Entrypoint)
	}
...
func TestRegister(t *testing.T) {
	...
	plugin := plugins.GetEmbeddedPlugin("tool.libxml2")
	if plugin == nil {
	...
	if plugin.Manifest.PluginID != "tool.libxml2" {
		t.Errorf("expected PluginID 'tool.libxml2', got '%s'", plugin.Manifest.PluginID)
	}
...
func TestRegisterMultipleCalls(t *testing.T) {
	...
	for _, p := range pluginsList {
		if p.Manifest.PluginID == "tool.libxml2" {
			libxml2Count++
		}
	}
	if libxml2Count != 1 {
		t.Errorf("expected 1 libxml2 plugin in registry, got %d", libxml2Count)
	}
}
...
func TestHandlerExecute(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args:            nil,
			expectError:     true,
			expectedErrText: "libxml2 command '' requires external plugin",
		},
		{
			name:            "xmllint command",
			command:         "xmllint",
			args:            map[string]interface{}{"input": "test.xml"},
			expectError:     true,
			expectedErrText: "libxml2 command 'xmllint' requires external plugin",
		},
		{
			name:            "xmlcatalog command",
			command:         "xmlcatalog",
			args:            map[string]interface{}{"file": "catalog.xml"},
			expectError:     true,
			expectedErrText: "libxml2 command 'xmlcatalog' requires external plugin",
		},
		{
			name:            "any command",
			command:         "any-command",
			args:            map[string]interface{}{},
			expectError:     true,
			expectedErrText: "libxml2 command 'any-command' requires external plugin",
		},
		{
			name:            "nil args",
			command:         "test",
			args:            nil,
			expectError:     true,
			expectedErrText: "libxml2 command 'test' requires external plugin",
		},
		{
			name:            "empty args",
			command:         "test",
		...
		},
		{
			...
			expectError:     true,
			expectedErrText: "libxml2 command 'process' requires external plugin",
		},
	}
...
func TestHandlerExecuteCommandVariations(t *testing.T) {
	...
	for _, cmd := range commands {
		t.Run(cmd, func(t *testing.T) {
			...
			if err != nil {
				expectedErr := "libxml2 command '" + cmd + "' requires external plugin"
...
func TestHandlerIntegration(t *testing.T) {
	...
	resp, err := plugins.ExecuteEmbeddedPlugin("tool.libxml2", &plugins.IPCRequest{
		Command: "xmllint",
		Args:    map[string]interface{}{"input": "test.xml"},
	})
...
func TestPluginRegisteredByDefault(t *testing.T) {
	...
	if !plugins.HasEmbeddedPlugin("tool.libxml2") {
		t.Error("expected plugin to be registered after calling Register")
	}

	plugin := plugins.GetEmbeddedPlugin("tool.libxml2")
	if plugin == nil {
		t.Fatal("expected to retrieve libxml2 plugin")
	}
