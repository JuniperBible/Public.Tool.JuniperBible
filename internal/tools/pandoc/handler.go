// Package pandoc provides the embedded handler for pandoc tool.
package pandoc
...
// Handler implements the EmbeddedToolHandler interface for pandoc.
type Handler struct{}

// Manifest returns the plugin manifest for registration.
func Manifest() *plugins.PluginManifest {
	return &plugins.PluginManifest{
		PluginID:   "tool.pandoc",
		Version:    "1.0.0",
		Kind:       "tool",
		Entrypoint: "tool-pandoc",
...
func (h *Handler) Execute(command string, args map[string]interface{}) (interface{}, error) {
	return nil, fmt.Errorf("pandoc command '%s' requires external plugin", command)
}
