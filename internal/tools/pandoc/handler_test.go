...
func TestManifest(t *testing.T) {
	...
	// Verify plugin ID
	if manifest.PluginID != "tool.pandoc" {
		t.Errorf("expected PluginID 'tool.pandoc', got '%s'", manifest.PluginID)
	}
	...
	// Verify entrypoint
	if manifest.Entrypoint != "tool-pandoc" {
		t.Errorf("expected Entrypoint 'tool-pandoc', got '%s'", manifest.Entrypoint)
	}
...
func TestRegister(t *testing.T) {
	...
	if !plugins.HasEmbeddedPlugin("tool.pandoc") {
		t.Fatal("expected plugin to be registered")
	}

	// Retrieve and verify the plugin
	ep := plugins.GetEmbeddedPlugin("tool.pandoc")
	if ep == nil {
	...
	if ep.Manifest.PluginID != "tool.pandoc" {
		t.Errorf("expected PluginID 'tool.pandoc', got '%s'", ep.Manifest.PluginID)
	}
...
func TestExecute(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			command: "convert",
			args:    map[string]interface{}{"input": "file.md", "output": "file.html"},
		},
		{
			name:    "html to markdown",
			command: "html2md",
			args:    map[string]interface{}{"file": "input.html"},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !strings.Contains(errMsg, "pandoc") {
				t.Errorf("expected error to mention 'pandoc', got: %s", errMsg)
			}
...
func TestExecuteErrorFormat(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			command:     "convert",
			expectedMsg: "pandoc command 'convert' requires external plugin",
		},
		{
			command:     "html2md",
			expectedMsg: "pandoc command 'html2md' requires external plugin",
		},
		{
			command:     "",
			expectedMsg: "pandoc command '' requires external plugin",
		},
	}
...
func TestExecuteViaEmbeddedPlugin(t *testing.T) {
	...
	resp, err := plugins.ExecuteEmbeddedPlugin("tool.pandoc", req)
	if err != nil {
...
func TestExecuteViaEmbeddedPluginMultipleCommands(t *testing.T) {
	...
	commands := []string{"convert", "html2md", "md2html", "list", "unknown", ""}
