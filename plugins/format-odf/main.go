...
func handleEnumerate(args map[string]interface{}) {
	...
	r, err := zip.OpenReader(path)
	if err != nil {
		respondError(fmt.Sprintf("failed to open ODF: %v", err))
		return
	}
	defer r.Close()
...
func parseODFContent(content, artifactID string) []*Document {
	doc := &Document{
		ID:    artifactID,
		Title: artifactID,
		Order: 1,
...
func handleEmitNative(args map[string]interface{}) {
	...
	if raw, ok := corpus.Attributes["_odf_raw"]; ok && raw != "" {
		...
		if err == nil {
			if err := os.WriteFile(outputPath, rawData, 0644); err != nil {
				respondError(fmt.Sprintf("failed to write ODF: %v", err))
				return
			}
	...
	if err := generateODF(outputPath, &corpus); err != nil {
		respondError(fmt.Sprintf("failed to generate ODF: %v", err))
		return
	}
...
func generateODF(outputPath string, corpus *Corpus) error {
