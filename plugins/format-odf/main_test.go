...
func createTestODF(t *testing.T, path string) {
	t.Helper()

	zipFile, err := os.Create(path)
	if err != nil {
		t.Fatalf("failed to create ODF file: %v", err)
	}
	defer zipFile.Close()
	...
	// Write manifest
	manifestWriter, _ := zipWriter.Create("META-INF/manifest.xml")
...
<manifest:manifest xmlns:manifest="urn:oasis:names:tc:opendocument:xmlns:manifest:1.0">
  ...
  <manifest:file-entry manifest:full-path="styles.xml" manifest:media-type="text/xml"/>
	...
	// Write content
	contentWriter, _ := zipWriter.Create("content.xml")
	contentWriter.Write([]byte(`<?xml version="1.0" encoding="UTF-8"?>
<office:document-content xmlns:office="urn:oasis:names:tc:opendocument:xmlns:office:1.0" xmlns:text="urn:oasis:names:tc:opendocument:xmlns:text:1.0">
  <office:body>
    <office:text>
      <text:h text:style-name="BookTitle">Genesis</text:h>
      <text:h text:style-name="ChapterTitle">Chapter 1</text:h>
      <text:p text:style-name="Verse"><text:span text:style-name="VerseNum">1</text:span> In the beginning God created the heavens and the earth.</text:p>
      <text:p text:style-name="Verse"><text:span text:style-name="VerseNum">2</text:span> And the earth was without form and void.</text:p>
    </office:text>
  </office:body>
</office:document-content>`))

	// Write styles
	stylesWriter, _ := zipWriter.Create("styles.xml")
	stylesWriter.Write([]byte(`<?xml version="1.0" encoding="UTF-8"?>
<office:document-styles xmlns:office="urn:oasis:names:tc:opendocument:xmlns:office:1.0" xmlns:style="urn:oasis:names:tc:opendocument:xmlns:style:1.0">
  <office:styles>
    <style:style style:name="BookTitle" style:family="paragraph"/>
...
// TestODFDetect tests the detect command.
func TestODFDetect(t *testing.T) {
	tmpDir, err := os.MkdirTemp("", "odf-test-*")
	if err != nil {
	...
	if result["format"] != "ODF" {
		t.Errorf("expected format ODF, got %v", result["format"])
	}
}

// TestODFDetectNonODF tests detect command on non-ODF file.
func TestODFDetectNonODF(t *testing.T) {
	tmpDir, err := os.MkdirTemp("", "odf-test-*")
	if err != nil {
		t.Fatalf("failed to create temp dir: %v", err)
	}
	...
	if result["detected"] == true {
		t.Error("expected detected to be false for non-ODF file")
	}
}

// TestODFExtractIR tests the extract-ir command.
func TestODFExtractIR(t *testing.T) {
	tmpDir, err := os.MkdirTemp("", "odf-test-*")
	if err != nil {
		t.Fatalf("failed to create temp dir: %v", err)
	}
...
// TestODFEmitNative tests the emit-native command.
func TestODFEmitNative(t *testing.T) {
	tmpDir, err := os.MkdirTemp("", "odf-test-*")
	if err != nil {
		t.Fatalf("failed to create temp dir: %v", err)
	}
	...
	odfPath, ok := result["output_path"].(string)
	if !ok {
		t.Fatal("output_path is not a string")
	}

	// Verify the output file is a valid zip
	zipReader, err := zip.OpenReader(odfPath)
	if err != nil {
	...
	for _, f := range zipReader.File {
		if f.Name == "content.xml" {
			...
			rc.Read(content)
			rc.Close()
			if !strings.Contains(string(content), "In the beginning.") {
				t.Error("expected verse content in content.xml")
			}
		}
	}

	if !hasContent {
		t.Error("expected content.xml in ODF")
	}
}

// TestODFRoundTrip tests L0 lossless round-trip via raw storage.
func TestODFRoundTrip(t *testing.T) {
	...
	odfPath := filepath.Join(tmpDir, "original.odt")
	createTestODF(t, odfPath)

	originalData, _ := os.ReadFile(odfPath)
...
// TestODFIngest tests the ingest command.
func TestODFIngest(t *testing.T) {
	tmpDir, err := os.MkdirTemp("", "odf-test-*")
	if err != nil {
		t.Fatalf("failed to create temp dir: %v", err)
	}
