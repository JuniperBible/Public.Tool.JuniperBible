...
func extractOnlineBibleContent(content, artifactID string) []*Document {
	// Group verses by book
	books := make(map[string][]*ContentBlock)
	...
	if raw, ok := corpus.Attributes["_onlinebible_raw"]; ok && raw != "" {
		...
		if err == nil {
			if err := os.WriteFile(outputPath, rawData, 0600); err != nil {
				respondError(fmt.Sprintf("failed to write OnlineBible: %v", err))
				return
			}
