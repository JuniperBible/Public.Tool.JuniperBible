...
// parsingContext holds the state during USFM parsing
...
// newParsingContext creates a new parsing context with initialized corpus
func newParsingContext(content string) *parsingContext {
	ctx := &parsingContext{
		corpus: &Corpus{
			Version:      "1.0.0",
			ModuleType:   "BIBLE",
	...
	ctx.corpus.Attributes["_usfm_raw"] = content
	return ctx
}

// parseMarkerLine extracts marker and value from a USFM line
func parseMarkerLine(line string) (marker string, value string, ok bool) {
...
func handleBookID(ctx *parsingContext, value string) {
...
// handleHeaderTOC processes header and TOC markers
func handleHeaderTOC(ctx *parsingContext, marker, value string) {
...
// handleTitle processes main title markers
func handleTitle(ctx *parsingContext, marker, value string) {
...
// handleChapter processes chapter markers
func handleChapter(ctx *parsingContext, value string) {
	if matches := chapterRegex.FindStringSubmatch(value); len(matches) > 0 {
		ctx.currentChapter, _ = strconv.Atoi(matches[1])
	}
}

// handleVerse processes verse markers and creates content blocks
func handleVerse(ctx *parsingContext, value string) {
	if ctx.currentDoc == nil {
		return
	}
...
// handleParagraph processes paragraph markers
func handleParagraph(ctx *parsingContext, marker, value string) {
	if ctx.currentDoc == nil || value == "" {
		return
	}
...
// handlePoetry processes poetry markers
func handlePoetry(ctx *parsingContext, marker, value string) {
	if ctx.currentDoc == nil || value == "" {
		return
	}
...
// processUSFMLine processes a single line of USFM content
func processUSFMLine(ctx *parsingContext, line string) {
	if strings.TrimSpace(line) == "" {
		return
	}

	marker, value, ok := parseMarkerLine(line)
	if !ok {
		return
	}
	...
	case "h", "toc1", "toc2", "toc3":
		handleHeaderTOC(ctx, marker, value)
	case "mt", "mt1", "mt2", "mt3":
		handleTitle(ctx, marker, value)
	case "c":
		handleChapter(ctx, value)
	case "v":
		handleVerse(ctx, value)
	case "p", "m", "pi", "mi", "nb":
		handleParagraph(ctx, marker, value)
	case "q", "q1", "q2", "q3", "qr", "qc", "qm":
		handlePoetry(ctx, marker, value)
	}
}

// parseUSFMToIR converts USFM text to IR Corpus
func parseUSFMToIR(data []byte) (*Corpus, error) {
	ctx := newParsingContext(string(data))

	scanner := bufio.NewScanner(bytes.NewReader(data))
	for scanner.Scan() {
		processUSFMLine(ctx, scanner.Text())
	}
