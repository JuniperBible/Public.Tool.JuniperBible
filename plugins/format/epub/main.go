...
func handleEnumerate(args map[string]interface{}) {
	...
	r, err := zip.OpenReader(path)
	if err != nil {
		ipc.RespondErrorf("failed to open EPUB: %v", err)
		return
	}
	defer r.Close()
...
func parseEPUBContent(r *zip.ReadCloser, opfPath string) []*ipc.Document {
...
func handleEmitNative(args map[string]interface{}) {
	...
	if raw, ok := corpus.Attributes["_epub_raw"]; ok && raw != "" {
		...
		if err == nil {
			if err := os.WriteFile(outputPath, rawData, 0600); err != nil {
				ipc.RespondErrorf("failed to write EPUB: %v", err)
				return
			}
	...
	if err := generateEPUB(outputPath, &corpus); err != nil {
		ipc.RespondErrorf("failed to generate EPUB: %v", err)
		return
	}
	ipc.MustRespond(&ipc.EmitNativeResult{
		OutputPath: outputPath,
...
func generateEPUB(outputPath string, corpus *ipc.Corpus) error {
...
func generateXHTML(doc *ipc.Document, language string) string {
	if language == "" {
		language = "en"
	}
