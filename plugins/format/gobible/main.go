...
func extractGoBibleContent(data []byte, artifactID string) []*ipc.Document {
	doc := &ipc.Document{
		ID:    artifactID,
		Title: artifactID,
		Order: 1,
	...
	if raw, ok := corpus.Attributes["_gobible_raw"]; ok && raw != "" {
		...
		if err == nil {
			if err := os.WriteFile(outputPath, rawData, 0644); err != nil {
				ipc.RespondErrorf("failed to write GoBible: %v", err)
				return
			}
...
func createCollectionsFile(corpus *ipc.Corpus) []byte {
	var buf bytes.Buffer
...
func createBookDataFile(doc *ipc.Document) []byte {
	var buf bytes.Buffer
