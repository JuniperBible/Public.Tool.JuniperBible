...
func TestOSHBEmitNative(t *testing.T) {
	tmpDir, _ := os.MkdirTemp("", "oshb-test-*")
	defer os.RemoveAll(tmpDir)
	corpus := ipc.Corpus{ID: "test", Title: "Test", Documents: []*ipc.Document{{ID: "Gen", Title: "Genesis", Order: 1, ContentBlocks: []*ipc.ContentBlock{{ID: "cb-1", Sequence: 1, Text: "בְּרֵאשִׁית"}}}}}
