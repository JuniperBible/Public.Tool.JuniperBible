...
func Detect(path string) (*ipc.DetectResult, error) {
	...
	if ext == ".txt" || ext == ".tsv" {
		...
		if err != nil {
			return &ipc.DetectResult{Detected: false, Reason: "failed to read file"}, nil
		}
		content := string(data)
...
// Parse reads an SBLGNT file and converts it to IR Corpus.
func Parse(path string) (*ir.Corpus, error) {
	...
	corpus := &ir.Corpus{
		...
		LossClass:    "L1",
		Attributes:   map[string]string{"_sblgnt_raw": hex.EncodeToString(data)},
	}

	corpus.Documents = extractContent(string(data), artifactID)
	if len(corpus.Documents) == 0 {
		corpus.Documents = []*ir.Document{{ID: artifactID, Title: artifactID, Order: 1}}
	}

	return corpus, nil
}

// extractContent parses SBLGNT text content and extracts documents with content blocks.
func extractContent(content, artifactID string) []*ir.Document {
	...
	for verseRef, words := range verseWords {
		...
		if _, exists := bookDocs[book]; !exists {
			bookDocs[book] = &ir.Document{ID: book, Title: book, Order: bookOrder[book]}
		}
...
// Emit converts an IR Corpus back to SBLGNT format.
func Emit(corpus *ir.Corpus, outputDir string) (string, error) {
	outputPath := filepath.Join(outputDir, corpus.ID+".txt")

	// If we have the raw SBLGNT data preserved, emit it for L0 lossless round-trip
	...
	// Otherwise reconstruct from IR (L1)
