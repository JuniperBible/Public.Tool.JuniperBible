...
func TestSFMEmitNative(t *testing.T) {
	tmpDir, _ := os.MkdirTemp("", "sfm-test-*")
	defer os.RemoveAll(tmpDir)
	corpus := ipc.Corpus{ID: "test", Title: "Test", Documents: []*ipc.Document{{ID: "GEN", Title: "Genesis", Order: 1, ContentBlocks: []*ipc.ContentBlock{{ID: "cb-1", Sequence: 1001, Text: "In the beginning"}}}}}
