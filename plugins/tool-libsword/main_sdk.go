...
// loadLibswordRequest loads the extended request with SWORD-specific fields.
...
// executeWithTranscript wraps profile execution with transcript handling.
// Uses custom request type for SWORD-specific fields.
func executeWithTranscript(req *LibswordRequest, config *ipc.ToolConfig) {
	...
	// Create base request for handler
	baseReq := &ipc.ToolRunRequest{
		Profile: req.Profile,
		Args:    req.Args,
		OutDir:  req.OutDir,
	}
	// Pass SWORD-specific fields via Args
	if baseReq.Args == nil {
